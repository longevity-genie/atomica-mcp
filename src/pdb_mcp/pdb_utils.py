...
- Loading and querying PDB annotations
...
# Global dictionaries for local PDB annotations
...
def load_pdb_annotations(
    ...
    """
    Load PDB annotation TSV files into global dataframes for fast lookup.
...
def get_uniprot_ids_from_tsv(pdb_id: str, chain_id: str, pdb_uniprot_data: Optional[pl.DataFrame] = None) -> List[str]:
    """
    Get UniProt IDs for a PDB chain from TSV data.
...
def get_organism_from_tsv(pdb_id: str, chain_id: str, pdb_taxonomy_data: Optional[pl.DataFrame] = None, anage_data: Optional[Dict[str, Dict[str, Any]]] = None) -> Dict[str, Any]:
    """
    Get organism information for a PDB chain from TSV data.
    ...
    Args:
        ...
        anage_data: Optional AnAge data dictionary. Uses global if not provided.
    ...
    known_names: Set[str] = set(anage.keys())  # All scientific names in AnAge (lowercase)
